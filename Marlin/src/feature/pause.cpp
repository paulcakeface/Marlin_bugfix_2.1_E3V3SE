...
bool load_filament(const float slow_load_length/*=0*/, const float fast_load_length/*=0*/, const float purge_length/*=0*/, const int8_t max_beep_count/*=0*/,
  ...
  if (pause_for_user) {
    ...
    while (marlin.wait_for_user) {
      impatient_beep(max_beep_count);
          ...
          #define _CASE_INSERTED(N) case N-1: if (!FILAMENT_IS_OUT(N)) marlin.wait_for_user = false; break;
          switch (motion.extruder) {
            REPEAT_1(NUM_RUNOUT_SENSORS, _CASE_INSERTED)
          }
        #else
          if (!FILAMENT_IS_OUT()) marlin.wait_for_user = false;
        #endif
      #endif
  ...
  if (fast_load_length) {
    ...
    marlin.wait_for_user = true; // A click or M108 breaks the purge_length loop
...
void wait_for_confirmation(const bool is_reload/*=false*/, const int8_t max_beep_count/*=0*/ DXC_ARGS) {
  ...
  while (marlin.wait_for_user) {
    ...
    if (nozzle_timed_out) {
      ...
      IF_DISABLED(PAUSE_REHEAT_FAST_RESUME, marlin.wait_for_user = true);
